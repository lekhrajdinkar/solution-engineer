## 2025 :: Brain dump

### Dec

### Nov

### Oct

### sep

### Aug

### July

| **Category / Task**                        | **Details**                                                                                                                                     |
|--------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|
| 🧩 **Issue Resolution & Production Support** |
| Prod Issue 3                               | JSON parsing failed. Doc said `int`, got `long`. Senior suggested `Long`, you recommended `bigint`. Found unwanted attribute → raised backlog JIRA. |
| Prod Issue 2                               | Blank file ("Noise") received. Coordinated with upstream `app-4` to resolve.                                                                    |
| Prod API                                   | Prod API called with scope: `execute`, `read`, `datamod`. RBAC role set with scope `test`.                                                      |
| Mock API for QA                            | Created to simulate partner response for testing Prod 3 fix.                                                                                    |
| Harness Pipeline Status                    | Now visible in GitHub Desktop. Experience shared.                                                                                               |
| DR Script Validation                       | Post-production validation for new Model launch.                                                                                                |
| Business Query                             | Clarified T+2 / T+3 logic. Investigated failed SNS alert.                                                                                       |
| 🔐 **Security & Infrastructure**               |
| Wildcard Certificate on Oz                 | Created wildcard cert. Easier to renew. Used OpenSSL, Nomos. Manual AWS import across regions (no ACM replication). Documented process.         |
| New VPC Request                            | Requested with engine team (max 5).                                                                                                             |
| SSL on Kafka                               | Discussed SSL on container vs local setup in offshore sync.                                                                                     |
| Kafka Onboarding (app-4)                   | Helped app-4 with Kafka, 404/network issue, Conduktor UI, consumer groups, Okta setup.                                                          |
| 🔄 **Project/Release Management**              |
| SACA Project                               | Swift Alliance Cloud Adapter: Kafka topics for MT/MX (no Avro), 7-yr retention. Release conflict with ISO (Nov 2025), moved to Q1 2026.         |
| Common Kafka Stack                         | Designed for SACA, COMET, OIS using common Okta ID.                                                                                             |
| DB Deployment                              | Did DB deployment in senior’s absence. Taught Yubo with Flyway ✔️                                                                               |
| New Model Launch                           | Reviewed script. Planning deployment steps.                                                                                                     |
| Offshore Test Planning                     | Coordinated DVA training and covered full test areas.                                                                                           |
| 📦 **Data & ETL Support**                      |
| ETL File Support                           | Helped E2E team arrange latest files from S3 ETL bucket.                                                                                        |
| 🔌 Integration & Collaboration             |
| Kafka Bridge with COMET ✔️                 | Guided developer (Poland timezone), explained topic replication, used screenshots, interactive steps. Helped scope the right scenario.          |
| SACA/COMET/OIS Meetings                    | Participated in planning meetings. Shared progress, aligned delivery timelines, supported parallel migrations.                                  |

```
=== dumps ===
- **Fixed Prod 3 issue**
    - new mock api to mock partner response for QA team to test changes
    - RBAC role : scope - test
    - **prod API called**, having scope - execute, read, datamod
- exp shared: asked tell that now can see **harness pipeline status on github desktop**
- **prod issue 3**
    - json response processing failed. type was int as per doc but received long value.
    - senior advised to change to Lonf type from int.
    - i advised bigint.
    - also did code analysis and found unwanted attribute, create backlog jira for 
- **generate wild card certificate** on oz.
    - easy to maintain/re-new
    - documented : **openssl**, **nomos**, import from aws region1 to region2 --> have to do manually/cli. no aws replication for ACM.
- **SACA** project -> swift alliance cloud adaptor
    - MT  topic1, MX topic2, on kafka
    - plain text message, no avro schema
    - 7 years retention on prod
    - follow topic name as per template
    - discussion on release date: dec2025, conflict with iso nov2025, wont be possible to do 2 parallel
    - also migrate along  comet, ois, saca, === 3 kafka parallel project
    - wrote common stack for all 3 three. design : use common okta id
    - shifting to Q1 2026
- production release : DR scripts validation post production for **new Model launch**
- **help e2e team to arrange latest's file from etl s3 file**
- **Prod-issue 2**: received blank file: Noise. requested app-4 upstream to handle.
- **app-4 (gtri)** : shared knowledge on kafka onboarding, 404 issue, network issue, conducktor UI, consumer group id and okta setup
- **offshore**
    - **ssl** on container vs local on kafka 
    - aws dva training
    - test plan to cover all areas.
- in senior absence, did **DB deployment**, attended with meeting with SACA, comet, OIS teams
- **taught Yubo to DB deployment** - showed her flyway work ✔️
- reviews new model fund launch script and working on deployment plan
- **business query**
    - T+2 | T+3
    - SNS alert failed (happened)
- Talked with engine team  to create new VPC (max 5)
- **Kafka bridge setup with COMET** ✔️
      -  collaborating with developer  on polland timezone.
      - explained him at high level about topic replilcation
      - he wsa not clear about next course of action.
      - took screenshot and showed him in **interactive way**
      - multiple scenario at org level.
      - asked him to check only scenario where we fit
      - IT 2 IT requests```
```

### June ✅

| **Category**        | **Details**                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Kafka & Infra**   | - Provisioned Kafka topics via DevHub (moved from Streamhub UI) <br> - Fixed SSL issue in PoC; advised offshore to switch JKS to PKCS12 <br> - Reported Kafka access issue from home to Nerve Center ✔️ <br> - Setup Conduktor UI for schema validation and dummy data <br> - Created IaC + pipeline and taught offshore ✔️ <br> - Updated IaC with env vars for DR; refactored to be reusable across modules <br> - Built Kafka services in Java and Python |
| **API Integration** | - Collaborated with MuleSoft to FastAPI (ECS/ALB) migration ✔️ <br> - Designed release plan to go live 2 weeks after MuleSoft <br> - Added fallback mechanism using flag switch <br> - Followed up for prod URL & Okta <br> - Reviewed and validated property file                                                                                                                                                                                           |
| **Learning**        | - Angular 17 brush-up + Signals 📚 <br> - GenAI PoC: Prompt engineering + RAG 📚                                                                                                                                                                                                                                                                                                                                                                             |

```
- **kafka**
    - kafka topic provision + read devHub
    - old : streamhub UI
    - issue from home, informed nerve center team. ✔️
    - poc project on kafka with SSL fix.
    - appraised offshore to fix, change JKS store format to PKS12.
    - conductor UI setup. asked offshore to use it for schema validation and dummy send.
    - created IAC and pipeline, taught offshore and asked them to other env. ✔️
    - updated IAC with env var DR deployment
    - refactor to use for all module
    - services  in java and python
- **ISO validation / observation**
- `collaborating` with ** mulesoft api to fast api ECS/ALB** ✔️
    - created release plan, going 2 weeks after them, to let things stabilize
    - fallback option, flag switch to switch back
    - follow up on prod url + okta setup
    - reviewing and validating  prop file.
- **Angular 17: brush up + signals** 📚
- **genAI poc** : prompt engineering + RAG 📚
```

### may ✅

| **Category**             | **Details**                                                                                                                                                                                                                                                                  |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **API & Auth**           | - Migrated from MuleSoft to AWS FastAPI <br> - Setup Okta and handled release coordination <br> - Implemented token refresh in singleton bean ✔️ <br> - Fixed `415` error (duplicate content-type) ✔️ <br> - Simplified config by removing subclass and using inferred beans |
| **SWIFT / ISO**          | - Gained access to SWIFT sparing + translator tools <br> - Collaborated with Yov on FSR and ISO translation understanding                                                                                                                                                    |
| **Swift Alliance Cloud** | - Designed workflow on Confluence <br> - Setup channel certificate, AWS Lambda to fetch access key <br> - Read SWIFT API docs + Postman tests <br> - Created dev JIRA, designed API calls <br> - Setup sandbox & pilot environments                                          |

```
- **mulesoft migration to AWS fast api**
    - setup okta 
    - release management / collaborated with partner team
    - okta refresh API : updated
    - code : **token refresh was done in singleton bean** ✔️
    - **fixed 415** : content type was present twice ✔️
    - infer bean from properties | simplified by removing subclass ✔️
- **Swift ISO changes**
    - got access on developer tool - SWIFT sparing tool + translator
    - worked with Yov on FSR, to understand SWIFT/ISO translation flow.
- **Swift alliance cloud** :: designed workflow on confluence
    - channel certificate setup 
    - aws lambda to get access key 
    - read SWIFT API doc  + tried on postman
    - Created development JIRA designed API call  
    - sandbox and pilot environment setup
```

### april ✅

| **Category**      | **Details**                                                                                                                                                |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Kafka Kickoff** | - Refreshed Kafka knowledge; compared with KDS and AMK <br> - Created internal notes/blogs <br> - Ran PoC with Docker Compose (Apache Kafka + Spring Boot) |

```
- **Kafka kick off** : did training and brush up
    - Compared with KDS and AMK
    - Create notes/blogs 
    - **poc** : docker compose - ran locally apache kafka + SB project
```

### march ✅

| **Category**       | **Details**                                                                           |
| ------------------ | ------------------------------------------------------------------------------------- |
| **Gateway & Auth** | - Setup API Gateway with Sourabh ✔️ <br> - Integrated ACM certs and Lambda Authorizer |
| **Networking**     | - Configured Transient Gateway (VPC Link → ALB → on-prem API using RAM)               |

```
- **API gateway setup** | sourabh N ✔️
- ACM and lambda Authorizer
- **Transient gateway** | RAM
    - VPC link :: ALB > on-prem API
```

### feb ✅

| **Category**                  | **Details**                                                                                          |
| ----------------------------- | ---------------------------------------------------------------------------------------------------- |
| **IAM & Connection Handling** | - IAM role expired after 15 mins; impacted BATCH <br> - Forced connection pool size to 15 in SB code |
| **RBAC & Security**           | - Suggested method-level authorization using scopes <br> - Highlighted dynamic vulnerability risks   |

```
=== dumps ===
- IAM role expired after 15 min and impacting BATCH
- explicitly forcing connection pool connection in SB code to 15
- RBAC setup : Advised to setup method based authorization with scope | dynamic vulnerability
```
### jan

